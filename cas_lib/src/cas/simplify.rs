...
pub fn combine_like_terms(expression: &mut Node) -> (f64, Node) {
    let (coefficient, product) = match expression {
        Node::BinaryOp { left, operation, right } => {
            ...
            (lhs_coefficient * rhs_coefficient, Node::BinaryOp { left: Box::new(lhs_product), operation: OperationType::Multiply, right: Box::new(rhs_product) })
        },
...
//    }
    if coefficient == 1.0 {
        ...
        (coefficient, Node::BinaryOp { left: Box::new(Node::Number(coefficient)), operation: OperationType::Multiply, right: Box::new(product) })
    }
}